...
public class StagedDepthFirstSolver {
	...
	private static void descendFor(int depth, int depthLimit, TableauMoveIterator tmi, Queue<MoveTree> pmt) {
		MoveTree m = null;
		if (depth < depthLimit && tmi.hasNext()) {
			m = tmi.next();
			printTrace(tmi, m, pmt);
			descendFor(depth + 1, depthLimit, tmi, pmt);
	...
	private static void printTrace(TableauMoveIterator tmi, MoveTree m, Queue<MoveTree> q) {
		count += 1;
		if (count % 100000 == 0) {
			System.out.println(String.format("%8d: %s-%5d - %6d", count, m, tmi.checkedStates(), q.size()));
		}

		if (count % 1000000 == 0) {
			System.out.println(
					String.format("%8d:: %s-%5d\n%s", count, m, tmi.checkedStates(), m.resultingTableau(startTableau)));
		}
	}
